...
export default function Header() {
  ...
  return (
    <>
      ...
      >
        <div className="w-full px-16 sm:px-20 lg:px-32 xl:px-40">
          <div className="flex items-center justify-between h-20">
            ...
            <Link
              href="/"
              className={`text-2xl font-bold tracking-wider transition-colors duration-300 ${isScrolled ? 'text-gray-900' : 'text-gray-900'
                }`}
            >
              HOWDOYOUDO
            </Link>
      ...
      {/* Mobile Menu */}
      <div
        className={`fixed top-0 right-0 h-full w-80 bg-white z-50 transform transition-transform duration-300 ease-out md:hidden ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'
          }`}
      >
        <div className="p-6">
          ...
          <nav className="space-y-6">
            {navItems.map((item) => (
              <Link
                ...
                href={item.href}
                onClick={() => setIsMenuOpen(false)}
                className={`block text-lg font-medium transition-colors ${pathname === item.href
                    ? 'text-gray-900'
                    : 'text-gray-600 hover:text-gray-900'
                  }`}
              >
